...
int basecall::inferphasing(int cb)
{
    ...
    if(cb==0)
        ...
        var_pre=var_pha=0.002,var_stop=0.001,var_const=0,var_dim=0.997;
    else
    {
...
#pragma omp parallel for
            for(int j=0;j<block;j++)
                fixphasing(j),fixcolor(j);
#pragma omp parallel for
            for(int j=0;j<block;j++)
                phasing[j]=phasing_o[j],color[j]=color_o[j];
#pragma omp parallel for
            for(int j=0;j<block;j++)
                solve(j);
...
void basecall::init()
{
    ...
    cnstart=floor((float)cntotal/nblock*cnblock+0.5f);
    size[dcluster]=floor(((float)cntotal/nblock*(1+cnblock))+0.5f)-cnstart;
    ...
    for(int i=1;i<block;i++)
        previous.array().row(i)/=(bufferstart[i]>5?bufferstart[i]:5);
}
